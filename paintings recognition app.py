...
def feature_extractor_glcm(dataset):
    ...
    for image in range(dataset.shape[0]):  #iterate through each file 
        ...
        if(dataset.shape[0]==1):
            print("OK")
            string = ""
            string = "Energy= "+str(GLCM_Energy)+"\nContrast= "+str(GLCM_contr)+"\nCorrelation= "+str(GLCM_corr)+"\nDissimilarity= "+str(GLCM_diss)
            
            panel2 = Label(root,text=string)
...
def open_img(row_pos):
    ...
    # opens the image
    img = Image.open(x)
    code = x[x.find("__")+2:x.find(".")] # get number of image, images filesnames sholud be strictly like "'filename'__uniquenum.jpg"
    print(code)
...
#bool var to check if moldel already trained
TRAINED_TYPE = -1
def calculate_res(idx):
    ...
    if(var_check.get()==1):
        input_img_features = feature_extractor_glcm(input_img)
    if(var_check.get()==2):
    ...
    if(TRAINED_TYPE == -1 or TRAINED_TYPE != var_check.get()):
        ####################################################################
        #Extract features from training images
        if(var_check.get()==1):
            image_features = feature_extractor_glcm(x_train)
        if(var_check.get()==2):
